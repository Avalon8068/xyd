...
class TraceTest extends Component<any, any> {
  ...
  async loadData() {
    const ids: string[] = await storage.getIdsForKey('trace');
    storage.getBatchDataWithIds({key: 'trace', ids}).then(value => {
      if (value) {
        console.log(value);
        this.setState({
          loaded: true,
  ...
  async insertRandomValue() {
    ...
    storage
      .save({key: 'trace', id: `${id}`, data: value})
      .then(() => {
        Toast.info('插入成功');
        this.loadData();
      })
  ...
  renderItem = item => {
    return (
      <View style={{padding: 10}}>
        <Text>
          {item.value}{' '}
          <Button type={'warning'} onPress={() => this.removeById(item.id)}>
            删除
          </Button>
        </Text>
      </View>
  ...
  addEvent(taskId) {
    ...
    return new Promise((resolve, reject) => {
      console.log(`[BackgroundFetch]: ${taskId},${new Date().toString()}`);
      resolve();
    });
  }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.scan_info_box}>
          <Button onPress={() => this.initBackgroundFetch()}>记录轨迹</Button>
        </View>
        <Text>数据：</Text>
        {this.state.loaded && (
